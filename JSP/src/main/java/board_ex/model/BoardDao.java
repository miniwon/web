...
public class BoardDao {
	...
	/************************************************
	 * 함수명 : insert 역할 : 게시판에 글을 입력시 DB에 저장하는 메소드 인자 : BoardVO 리턴값 : 입력한 행수를 받아서 리턴
	 */
	public int insert(BoardVO rec) throws BoardException {
		...
		try {
			...
			String putQuery = " INSERT INTO BOARD_EX ( SEQ, TITLE, WRITER, CONTENT, REGDATE, CNT, PASS) "
					+ " VALUES ( SEQ_BOARDNO.nextval, ?, ?, ?, sysdate, 1, ? ) ";
			...
			String sql = " SELECT SEQ_BOARDNO.currval as seq FROM dual ";
	...
	/************************************************
	 * 함수명 : selectList 역할 : 전체 레코드를 검색하는 함수 인자 : 없음 리턴값 : 테이블의 한 레코드를 BoardVO 지정하고
	 * 그것을 ArrayList에 추가한 값
	 */
	...
	public BoardVO selectById(int seq) throws BoardException {
		...
		try {
			...
			con = DriverManager.getConnection(dbUrl, dbUser, dbPass);
			// * sql 문장만들기
			String sql = " SELECT seq, TITLE, WRITER, REGDATE, CONTENT, CNT FROM BOARD_EX WHERE SEQ = ? ";
